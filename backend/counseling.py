...
"""
    ...
    if previous_round:
        round_instruction = (
            ...
            "consider your own previous reply and the peer scoring notes, then produce an improved "
...
- Do not mention the five agents, scoring, or your character name.
- Do not use markdown.
...
def fallback_peer_scores(
    ...
    """Keep the app usable when a local model cannot produce JSON scores."""
    normalised = []
    for candidate in candidates:
        if candidate["character"] == judge_agent["character"]:
            continue
        ...
        row = {
            ...
            "note": f"Fallback score used because local LLM scoring failed: {reason[:120]}",
        }
        row["raw_total"] = sum(row[criterion["key"]] for criterion in RUBRIC)
        row["weighted_total"] = round(
            sum(row[criterion["key"]] * criterion["weight"] for criterion in RUBRIC),
...
def previous_round_summary(round_result: Dict[str, Any], current_character: str) -> str:
    ...
    score_lines = []
    for judge_result in round_result.get("peer_scores", []):
        if judge_result.get("judge") == current_character:
            continue
        ...
        own_score = next(
            (row for row in scores if row.get("character") == current_character),
            None,
        )
        own_note = own_score.get("note") if own_score else ""
        score_lines.append(
            f"- {judge_result.get('judge')}: preferred {top_score.get('character')} "
            f"({score_value(top_score)} weighted). Your previous reply scored "
            f"{score_value(own_score) if own_score else 0} weighted. {own_note or 'No direct note.'}"
        )
    ...
    return "\n".join([
        ...
        *(other_candidate_lines or ["- No other candidate replies."]),
        "Peer scoring notes:",
        *(score_lines or ["- No scoring notes."]),
        f"Previous winner: {winner.get('character', 'None')} - {winner.get('reply', '')}",
    ])
